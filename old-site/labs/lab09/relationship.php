...
<?php
  ...
  echo '<script type="text/javascript" src="resources/relationships.js"></script>'
...
<?php
  ...
  @ $db = new mysqli('localhost', 'zhengm4', 'zhengm4', 'iit');
  ...
  if ($havePost) {
    ...
    if ($errors != '') {
      ...
      if ($dbOk) {
        ...
        // checking for movie
        $checkQuery = "SELECT movieid 
                        FROM movies 
                        WHERE title = ? 
                            AND year = ?";
        $checkStatement = $db->prepare($checkQuery);
        $checkStatement->bind_param("ss",$titleForDb,$yearForDb);
        $checkStatement->execute();
        $checkStatement->store_result();

        if ($checkStatement->num_rows > 0) {
            $checkStatement->bind_result($movieid);
        ...
        // checking for actors
        $checkQuery = "SELECT actorid 
                        FROM actors 
                        WHERE last_name = ? 
                            AND first_names = ?";
        $checkStatement = $db->prepare($checkQuery);
        $checkStatement->bind_param("ss",$lastNameForDb,$firstNamesForDb);
        $checkStatement->execute();
        $checkStatement->store_result();

        if ($checkStatement->num_rows > 0) {
            $checkStatement->bind_result($actorid);
            $checkStatement->fetch();
        }
        else {
            ...
            $actorid = $statement->insert_id;
            $statement->execute();
        }

        $checkStatement->close();

        // check for the relationships between actors and their movies
        $checkQuery = "SELECT id 
                        FROM movie_actor 
                        WHERE actorid = ? 
                            AND movieid = ?";
        $checkStatement = $db->prepare($checkQuery);
        $checkStatement->bind_param("ss",$actorid,$movieid);
        $checkStatement->execute();
        $checkStatement->store_result();
        ...
        echo '<div class="messages"><h4>Success: ' . $statement->affected_rows . ' movie and actor information added to database.</h4>';
        echo $title . ' and ' . $firstNames . ' ' . $lastName . '</div>';
...
<h3>Add Movie</h3>
<form id="addForm" name="addForm" action="relationship.php" method="post" onsubmit="return validate(this);">
  <fieldset>
    <div class="formData">
      ...
      <div class="value"><input type="text" size="10" maxlength="10" value="<?php if($havePost && $errors != '') { echo $year; } ?>" name="year" id="year"/></div>
...
<?php
  if ($dbOk) {
    ...
    $query = 'SELECT movies.title, actors.last_name, actors.first_names
                FROM movies, actors, movie_actor
                WHERE movie_actor.movieid = movies.movieid
                    AND movie_actor.actorid = actors.actorid;';
    ...
    for ($i=0; $i < $numRecords; $i++) {
      ...
      echo '</td><td>';
      echo '<img src="resources/delete.png" class="delete deleteRelationship" width="16" height="16" alt="delete movie"/>';
      echo '</td></tr>';
